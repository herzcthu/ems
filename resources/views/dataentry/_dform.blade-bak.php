    ...
    @foreach($questions as $k => $question)
        <tr>
            @if($question->q_type == 'single')
                ...
                <td colspan="7">
                    <h4 class="{{ $question->question_number.'-question' }}"
                        lang="{!! Stevebauman\Translation\Facades\Translation::getLocale(); !!}">{{ _t($question->question) }}</h4>
                    ...
                    <div class="col-xs-offset-1">
                        @if($question->input_type == 'radio')
                            <ul class="radio" lang="{!! Stevebauman\Translation\Facades\Translation::getLocale(); !!}">
                                @foreach($question->answers as $answer_k => $answer_v)
                                    @if(!empty($answer_v))
                                        <li>
                                            {!! Form::radio("answers[$question->id]",$answer_k,null )
                                            !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                        </li>
                                    @endif
                                @endforeach
                        ...
                        @if($question->input_type == 'choice')
                            @foreach($question->answers as $answer_k => $answer_v)
                                @if(!empty($answer_v))
                                    {!! Form::checkbox("answers[$question->id]", $answer_k, null)
                                    !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                @endif
                            @endforeach
                        @endif
                        @if($question->input_type == 'select')
                            <div class="">
                                {!! Form::select("answers[$question->id]",array_filter($question->answers), null,
                                ['class' => 'form-control']) !!}
                            </div>
                        @endif
                        @if($question->input_type == 'text')
                            <div class="">
                                {!! Form::text("answers[$question->id]", array_key_exists('-99',
                                (array)$question->answers)? $question->answers[-99]:'No Answer', ['class' =>
                                'form-control']) !!}
                            </div>
                        @endif
            ...
            @if($question->q_type == 'main')
                ...
                <td colspan="7">
                    ...
                    @if($question->a_view == 'validated-table')
                        @foreach($question->answers as $answer_k => $answer_v)
                            @if(!empty($answer_v))
                                @if( in_array($answer_k, array('0', '-9')) )
                                    {!! Form::radio("answers[$children->id]",$answer_k )
                                    !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                @endif
                            @endif
                        @endforeach
                        <table id="validated-table" class="table">
                            ...
                            @if(is_array($question->answers))
                                ...
                                @endforeach
                                @foreach($question->answers as $answer_k => $answer_v)
                                    @if( !in_array($answer_k, array('-99', '-9')) )
                                                ...
                                                @if(!empty($answer_v))
                                                    <td>
                                                        @if($children->input_type == 'radio')
                                                            <div class="radio"
                                                                 lang="{!! Stevebauman\Translation\Facades\Translation::getLocale(); !!}">
                    ...
                    @else
                        @foreach($question->get_children as $children)
                            <div class="col-xs-offset-1">
                                <h4 class="{{ $children->question_number }}"
                                    lang="{!! Stevebauman\Translation\Facades\Translation::getLocale(); !!}">{{ $children->question_number }}
                                    . {{ _t($children->question) }}</h4>
                                @if($children->a_view == 'notes')
                                    {!! Form::text("notes[$children->id]", null, ['class' => 'form-control']) !!}
                                @endif
                                @if($children->a_view == 'categories')
                                    <div class="col-md-12">
                                        @foreach($question->answers as $answer_k => $answer_v)
                                            @if(!empty($answer_v))
                                                @if( in_array($answer_k, array('-99', '-8', '-9')) )
                                                    {!! Form::radio("answers[$children->id]",$answer_k )
                                                    !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                                @endif
                                            @endif
                            ...
                            @elseif(is_array($question->answers) && $children->q_type == 'same')
                                ...
                                @if($children->input_type == 'choice')
                                    @foreach($question->answers as $answer_k => $answer_v)
                                        @if(!empty($answer_v))
                                            {!! Form::checkbox("answers[$children->id]", $answer_k, null)
                                            !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                        @endif
                                    @endforeach
                                @endif
                                @if($children->input_type == 'select')
                                    <div class="">
                                        {!! Form::select("answers[$children->id]",array_filter($question->answers),
                                        null,['class' => 'form-control']) !!}
                                    </div>
                                @endif
                                @if($children->input_type == 'text')
                                    <div class="">
                                        {!! Form::text("answers[$children->id]", array_key_exists('-99',
                                        (array)$question->answers)? $question->answers[-99]:'No Answer', ['class' =>
                                        'form-control']) !!}
                                    </div>
                                @endif
                            ...
                            @else
                                ...
                                @if($children->input_type == 'choice')
                                    <div class="">
                                        @foreach($children->answers as $answer_k => $answer_v)
                                            @if(!empty($answer_v))
                                                {!! Form::checkbox("answers[$children->id]", $answer_k )
                                                !!} {{ $answer_k.' ('._t($answer_v).')' }}
                                            @endif
                                        @endforeach
                                    </div>
                                ...
                                @if($children->input_type == 'select')
                                    <div class="">
                                        {!!
                                        Form::select("answers[$children->id]",array_filter($children->answers), ['class'
                                        => 'form-control']) !!}
                                    </div>
                                @endif
                                @if($children->input_type == 'text')
        ...
        @foreach($questions as $k => $question)
            <tr>
                @if($question->q_type == 'spotchecker')
                    ...
                    <td><h4 class="{{ $question->get_parent->question_number.'-question' }}"
                            lang="{!! Stevebauman\Translation\Facades\Translation::getLocale(); !!}">{{ $question->get_parent->question_number }} {{ _t($question->get_parent->question) }}</h4>
                        Enumerator Answer: <p class="enuanswer" id="{{ $question->get_parent->question_number }}"></p>
                        <h5>Accuracy:</h5>
                        {!! Form::radio("accuaracy[$question->id]",1,null ) !!} Yes
                        {!! Form::radio("accuaracy[$question->id]",0,null ) !!} No
                    </td>
                    ...
                    <td>
                        ...
                        <div>
                            ...
                            @if($question->get_parent->input_type == 'text')
                                <div class="">
                                    {!! Form::text("answers[$question->id]", array_key_exists('-99',
                                    $question->answers)? $question->answers[-99]:'No Answer', ['class' =>
                                    'form-control']) !!}
                                </div>
                            @endif
