		...
		if (!empty($file)) {
			...
			foreach ($csv_array as $line) {
				...
				if (isset($line['state'])) {
					...
					$new_state = States::updateOrCreate(array('state' => $line['state'], 'state_id' => $state['state_id']), $state);
				...
				if (isset($line['district'])) {
					...
					$new_district = Districts::updateOrCreate(array('district' => $line['district'], 'states_id' => $district['states_id']), $district);
				...
				if (isset($line['township'])) {
					...
					$new_township = Townships::updateOrCreate(array('township' => $line['township'], 'districts_id' => $township['districts_id']), $township);
				...
				try {
					$new_village = Villages::updateOrCreate(array('village' => $village['village'], 'village_id' => $village_id, 'townships_id' => $village['townships_id']), $village);
